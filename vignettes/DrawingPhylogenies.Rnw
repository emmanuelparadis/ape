...
Section~\ref{sec:plotphylo}, and further graphical annotations can done with
...
tree or a large tree, on a small or a large device, the labels will
...
\noindent Although the four previous functions are not formally documented,
...
\noindent The second and third arguments are the two mostly
...
show how data can be shown in connection to the tips or to the branches.
...
there is recycling of the arguments. The values
...
and the user is asked to click on the plot to show where to draw the
scale bar.
...
which are then passed to a function of the \pkg{graphics} package giving a
...
integer specifying the number classes used to represent the data (if
\code{continuous = TRUE} then ten classes are used). Because we will
call \code{phydataplot} twice, we will obviously use the option
...
$y$-direction(s). The next bloc gives a function which uses the above
