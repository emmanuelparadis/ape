...
paper sheet and arrange them side by side to make a huge poster. This
would require (in meters):
...
is estimated to be $\approx 8.8\times 10^{26}$~m, so it is wide enough
to display our trees.
